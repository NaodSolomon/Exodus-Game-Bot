...
# Database path
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'upload', 'data.db')
ADMIN_DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'admin_dashboard', 'admin.db')
...
def logout():
    if 'admin_id' in session:
        ...
        admin_username = session['username'] if 'username' in session else 'Unknown'
...
def dashboard():
    ...
    SELECT strftime('%Y-%m', datetime(o.rowid, 'unixepoch')) as month, SUM(o.total_price) as revenue
    FROM orders o
    GROUP BY month
    ORDER BY month
    LIMIT 6
